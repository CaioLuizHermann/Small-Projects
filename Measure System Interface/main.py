...
def clean_entry_unit():
    if entry_unit.get() == "Number":
        entry_unit.delete(0, tk.END)
        entry_unit.config(fg="black")
def reset_entry_unit():
    if entry_unit.get() == "":
        entry_unit.insert(0, "Number")
        entry_unit.config(fg="gray")
def clean_entry_value():
    if entry_value.get() == "Quantity":
        entry_value.delete(0, tk.END)
        entry_value.config(fg="black")
def reset_entry_value():
    if entry_value.get() == "":
        entry_value.insert(0, "Quantity")
